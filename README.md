# Ankita-Shaw--Portfolio
Personal portfolio showcasing my AWS cloud projects, technical skills, and hands-on experience in building scalable solutions.
## Skills
- AWS (EC2, S3, IAM, Lambda)
- Data Analytics(Excel,SQL,NUMPY,PANDAS,MATPLOTLIB)
- Python / C++
- Git & GitHub

## Projects

### 1. Static Website on S3
- Hosted a website using AWS S3
- Configured public access and static hosting

### 2. EC2 Web Server
- Launched and configured an EC2 instance
- Deployed a basic web server

## Live Portfolio
🔗 [Click here to view my portfolio](https://Ankita-ux38.github.io//quiz-app)
🔗 [Click here to view my portfolio](https://Ankita-ux38.github.io/The-Vibin-Squad)
This Portfolio showcases my AWS projects and hands-on cloud experience.

## Contact
- Email: ankitashaw858@gmail.com
- GitHub: https://github.com/Ankita-ux38
